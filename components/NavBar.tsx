...
import { Heart, MessageCircle, User, Compass, LogOut, Star, Settings } from "lucide-react";
...
export default function NavBar() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("nav-counts")
  ...
  async function loadCounts() {
    ...
    // Approximate: likes received minus ones already swiped back
  ...
  return (
    <header className="bg-surface-800 border-b border-surface-600 sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 flex items-center justify-between h-14">
        <span className="font-serif text-xl text-gold-500 tracking-widest hidden sm:block">
          ELITEMATCH
        </span>
        ...
        <nav className="flex items-center gap-0.5">
          {NAV.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href}
                ...
                {badge > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 bg-gold-500 text-black
          ...
          <button onClick={signOut}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm
                       text-white/30 hover:text-white/60 transition-colors ml-1">
            <LogOut size={16} />
          </button>
        </nav>
      </div>
